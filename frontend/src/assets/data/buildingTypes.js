export default {
  tc: {
    short: 'ТЦ', full: 'Торговый центр', bg: 'primary', icon: 'blueDotIcon', buttonName: 'торговые центры',
  },
  mfk: {
    short: 'МФК',
    full: 'Многофункциональный комплекс',
    mobileFull: 'Многофункц. комплекс',
    bg: 'info',
    icon: 'violetDotIcon',
    buttonName: 'многофункциональные комплексы',
  },
  bc: {
    short: 'БЦ', full: 'Бизнес-центр', bg: 'success', icon: 'darkGreenDotIcon', buttonName: 'бизнес-центры',
  },
  ipn: {
    short: 'ИПН',
    full: 'Инвестиционный портфель недвижимости',
    mobileFull: 'Инвест. портфель недвиж.',
    bg: 'ipn',
    icon: 'nightDotIcon',
    buttonName: 'инвестиционные портфели недвижимости',
  },
  go: {
    short: 'ГО', full: 'Гостиница', bg: 'warning', icon: 'orangeDotIcon', buttonName: 'гостиницы',
  },
};
